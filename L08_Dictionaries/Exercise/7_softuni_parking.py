...
for _ in range(num_of_commands):
    ...
    if command[0] == "register":
        if command[1] in parking_dict:
            print(f"ERROR: already registered with plate number {parking_dict[command[1]]}")
        else:
            parking_dict[command[1]] = command[2]
            print(f"{command[1]} registered {command[2]} successfully")
    else:
        if command[1] not in parking_dict:
            print(f"ERROR: user {command[1]} not found")
        else:
            del parking_dict[command[1]]
...
#
# Output:
#   ohn registered CS1234JS successfully
